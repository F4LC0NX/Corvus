...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for CRV
    ...
    if (p_rewardSnapshot.strDistributionAsset != "CRV") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is CRV with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using CRV differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "CRV") {
        // Check amount
